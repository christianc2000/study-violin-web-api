...
<div class="relative inline-flex" x-data="{ open: false }">
    <button
        ...
        @auth
            ...
            @else
                <div class="absolute top-0 right-0 flex items-center justify-center bg-rose-500 dark:border-[#182235] rounded-full text-xxs text-white"
                    style="font-size: 9px; width: 14px; height: 14px;">
    ...
    {{-- <div class="origin-top-right z-10 absolute top-full -mr-48 sm:mr-0 min-w-80 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 py-1.5 rounded shadow-lg overflow-hidden mt-1 {{ $align === 'right' ? 'right-0' : 'left-0' }}"
        ...
        <ul>
            ...
            <li class="border-b border-slate-200 dark:border-slate-700 last:border-0">
                <a class="block py-2 px-4 hover:bg-slate-50 dark:hover:bg-slate-700/20" href="#0"
                    @click="open = false" @focus="open = true" @focusout="open = false">
                    <span class="block text-sm mb-2">🚀<span class="font-medium text-slate-800 dark:text-slate-100">Say
    ...
    <div class="origin-top-right z-10 absolute top-full -mr-48 sm:mr-0 min-w-80 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 py-1.5 rounded shadow-lg overflow-hidden mt-1 {{ $align === 'right' ? 'right-0' : 'left-0' }}"
        ...
        x-transition:leave="transition ease-out duration-200" x-transition:leave-start="opacity-100"
        x-transition:leave-end="opacity-0" x-cloak>
        <div class="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase pt-1.5 pb-2 px-4">Notificaciones
        </div>
        <ul>
            @forelse (Auth::user()->unreadNotifications as $notification)
                <li class="border-b border-slate-200 dark:border-slate-700 last:border-0">
                    <a class="block py-2 px-4 hover:bg-slate-50 dark:hover:bg-slate-700/20" href="#0"
                        @click="open = false" @focus="open = true" @focusout="open = false">
                        <span class="block text-sm mb-2">📣<span
                                class="font-medium text-slate-800 dark:text-slate-100">{{ $notification->data['title'] }}</span>
                            {{ $notification->data['content'] }}</span>
                        <span
                            class="block text-xs font-medium text-slate-400 dark:text-slate-500">{{ $notification->created_at->diffForHumans()}}</span> 
                    </a>
                </li>
                {{-- $notification->created_at->format('M d, Y')  --}}
            @empty
                <li class="border-b border-slate-200 dark:border-slate-700 last:border-0">
                    <a class="block py-2 px-4 text-sm text-center text-slate-400 dark:text-slate-500"
                        @click="open = false" @focus="open = true" @focusout="open = false">
            ...
            @endforelse
            <li class="border-b border-slate-200 dark:border-slate-700 last:border-0">
                <a class="block py-2 px-4 text-sm text-center text-slate-400 dark:text-slate-500" href="{{route('admin.notificacion.index')}}"
                    @click="open = false" @focus="open = true" @focusout="open = false">
